...
export default function DateTime({ type, value = '' }: DateTimeProps) {
  ...
  return (
    <div className="border-gray200 bg-white-bg flex w-[12rem] items-center gap-[0.8rem] rounded-[0.4rem] border px-[0.8rem] py-[1.2rem]">
      <span className="caption1-sb text-font-black-1">
        {type === 'date' ? '날짜' : '시간'}
      </span>
      <input
        ref={inputRef}
        type="text"
        className="body3-r text-font-gray-3 m-0 w-[7rem] overflow-hidden outline-none"
        value={input}
