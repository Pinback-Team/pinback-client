...
Figma MCP를 통해 피그마 디자인을 직접 참조하면서 컴포넌트를 구현한다.
...
`.env.local`에 추가한다 (커밋 금지, `.gitignore`에 이미 포함됨):
...
Codex 등 다른 툴을 쓰는 경우 해당 툴의 MCP 설정 파일에 동일한 서버 정의를 등록한다.

### 4. 연결 확인

세션에서 피그마 파일 URL을 공유하면 MCP가 자동으로 디자인 데이터를 로드한다.
